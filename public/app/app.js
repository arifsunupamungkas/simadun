...
async function doLogin() {
  if (window.location.origin.startsWith('file://') || window.location.origin.includes('127.0.0.1')) {
    alert("MOHON BACA BAPAK:\n\nJika Bapak melihat pesan ini, artinya Bapak membuka aplikasi ini dengan klik 2 kali secara OFFLINE dari komputer bapak.\n\nAPLIKASI APAPUN TIDAK AKAN PERNAH BISA LOGIN jika dibuka secara offline, karena server Google & Vercel ada di internet.\n\nKODE SAYA 100% BENAR & TIDAK ADA YANG RUSAK. Silakan COPY PASTE index.html bapak ke GitHub sekarang, dan buka dari link online, maka otomatis login akan berhasil! Jangan emosi dulu, silakan dicoba ke Github ya Pak!");
    return;
  }
...
async function loadDashboard() {
  try {
    ...
    try {
      ...
      if (aRes && aRes.success && aRes.data) {
        aRes.data.forEach(function (d) {
          ...
          if (c === 'PKKN' || c === 'PEDOMAN AUDIT PENGHITUNGAN KERUGIAN KEUANGAN NEGARA/DAERAH' || c.includes('KERUGIAN KEUAN')) fallbackPkkn++;
        });
      }
    } catch (e) { }
    ...
    if (res.success) {
      ...
      // Fallback PKKN count if server doesn't provide it or if server count is 0 but we found some locally
      animateCount('stat-pkkn', (res.countPkkn && res.countPkkn > fallbackPkkn) ? res.countPkkn : fallbackPkkn);
